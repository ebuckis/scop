...
/*   Updated: 2020/01/30 14:01:17 by kcabus      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
int		save_vertex(t_draw *draw, char **tab)
{
	...
	if (!draw->vbo_vertex.values)
		draw->vbo_vertex.values = malloc(sizeof(GLfloat)
			* draw->vbo_vertex.size);//TODO: verifier la taille
	else
		draw->vbo_vertex.values = realloc(draw->vbo_vertex.values,
			sizeof(GLfloat) * draw->vbo_vertex.size);
...
int		save_index(t_draw *draw, char **tab)
{
	...
	while (tab[i] && tab[i + 1] && tab[i + 2])
	{
		...
		if (!draw->vbo_index.values)
			draw->vbo_index.values =
				(GLshort *)malloc(sizeof(GLshort) * draw->vbo_index.size);//TODO: verifier la taille
		else
			draw->vbo_index.values =(GLshort *)realloc(draw->vbo_index.values,
				sizeof(GLshort) * draw->vbo_index.size);
		draw->vbo_index.values[draw->vbo_index.size - 3] = atoi(tab[1]) - 1;
		draw->vbo_index.values[draw->vbo_index.size - 2] = atoi(tab[i + 1]) - 1;
		draw->vbo_index.values[draw->vbo_index.size - 1] = atoi(tab[i + 2]) - 1;
		i++;
	}
	return (0);
}
